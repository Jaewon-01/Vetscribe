...
function ResultContent() {
  ...
  const tabLabel = (days: string) => `D-${days}`;
    ...
    <div className="min-h-screen bg-gray-50">
      {toast && (
        <div className="fixed top-5 left-1/2 -translate-x-1/2 z-50 bg-slate-900 text-white text-sm font-semibold px-5 py-3 rounded-2xl shadow-xl flex items-center gap-2">
          <span>✅</span>
          <span>{data.patientName} 보호자님께 문자가 발송되었습니다</span>
        </div>
      )}
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          >
            {copied ? (
              ...
              <><span className="text-xl">📋</span><span>복사하기</span></>
            )}
          </button>
          ...
          <button
            onClick={handleSend}
            disabled={sending}
            className="flex flex-col items-center justify-center gap-1.5 py-4 bg-slate-900 hover:bg-slate-800 disabled:bg-slate-400 border border-slate-900 rounded-2xl transition-all text-sm font-bold text-white shadow-sm"
          >
            {sending ? (
              <>
                ...
                <span className="text-xs">발송 중...</span>
              </>
            ) : (
